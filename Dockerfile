...
# Install Tailscale and 1Password CLI
RUN apt-get update -y && apt-get install -y tailscale 1password-cli \
  && apt-get clean && rm -f /var/lib/apt/lists/*_*

# ---- END DEPLOYMENT-SPECIFIC CONFIGURATION ----
...
# DEPLOYMENT-SPECIFIC: Run as `root` so we can run Tailscale in the startup script
# Replace with the following on a non-official deployment:
# USER nobody
USER root
