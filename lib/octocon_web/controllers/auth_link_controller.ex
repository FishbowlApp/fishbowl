defmodule OctoconWeb.AuthLinkController do
  ...
  plug :put_link_token
  plug Ueberauth, providers: [:google_link, :discord_link, :apple_link]
  ...
  def try_link_account("discord", conn, user, %{uid: discord_id}) do
    case Octocon.Accounts.link_discord_to_user(user, to_string(discord_id)) do
      ...
      {:ok, _} ->
        conn
        |> redirect(external: "https://fishbowl.systems/deep/link_success/discord")
    end
  end
  ...
  def try_link_account("google", conn, user, %{info: %{email: email}}) do
    case Octocon.Accounts.link_email_to_user(user, email) do
      ...
      {:ok, _} ->
        conn
        |> redirect(external: "https://fishbowl.systems/deep/link_success/google")
    end
  end

  def try_link_account("apple", conn, user, %{info: %{uid: apple_id}}) do
    case Octocon.Accounts.link_apple_to_user(user, apple_id) do
      {:error, :already_linked} ->
        conn
        |> put_status(403)
        |> json(%{
      ...
      {:ok, _} ->
        conn
        |> redirect(external: "https://fishbowl.systems/deep/link_success/apple")
    end
  end
end
