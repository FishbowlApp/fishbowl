defmodule OctoconWeb.AuthLinkController do
  ...
  def callback(%{assigns: %{ueberauth_auth: auth_data}} = conn, %{"provider" => provider}) do
    ...
    case Octocon.ClusterUtils.run_on_primary_no_endpoint(fn -> LinkTokenRegistry.get(link_token) end) do
      nil ->
        conn
      ...
      system_id when is_binary(system_id) ->
        ...
        Octocon.ClusterUtils.run_on_primary_no_endpoint(fn -> LinkTokenRegistry.delete(link_token) end)
