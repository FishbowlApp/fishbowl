defmodule Octocon.RPC.Postgres.Repo do
  defmacro __using__(opts) do
    quote bind_quoted: [opts: opts] do
      ...
      then it is suggested to use `Octocon.RPC.Postgres.rpc_and_wait/4` to perform the
      ...
      def __exec_on_primary__(func, args, opts) do
        ...
        if Keyword.get(opts, :await, default) do
          ...
          Octocon.RPC.Postgres.rpc_and_wait(@local_repo, func, args,
            rpc_timeout: rpc_timeout,
            replication_timeout: replication_timeout,
            tracker: Keyword.get(opts, :tracker)
          )
        else
          Octocon.RPC.NodeTracker.rpc_primary({@local_repo, func, args}, timeout: @timeout)
        end
      end
    end
  end
