defmodule Octocon.RPC.Postgres.LSN.Tracker do
  use GenServer
  require Logger
  ...
  """
  @spec get_last_replay(opts :: keyword()) :: nil | Octocon.RPC.Postgres.LSN.t()
  def get_last_replay(opts \\ []) do
    ...
    case :ets.lookup(table_name, :last_log_replay) do
      [{:last_log_replay, %Octocon.RPC.Postgres.LSN{} = stored}] ->
        stored
  ...
  """
  @spec replicated?(Octocon.RPC.Postgres.LSN.t(), opts :: keyword()) :: boolean()
  def replicated?(%Octocon.RPC.Postgres.LSN{source: :insert} = lsn, opts \\ []) do
    case get_last_replay(opts) do
      %Octocon.RPC.Postgres.LSN{} = stored ->
        Octocon.RPC.Postgres.LSN.replicated?(stored, lsn)
  ...
  """
  @spec request_notification(Octocon.RPC.Postgres.LSN.t(), opts :: keyword()) :: :ok
  def request_notification(%Octocon.RPC.Postgres.LSN{source: :insert} = lsn, opts \\ []) do
  ...
  """
  @spec await_notification(Octocon.RPC.Postgres.LSN.t(), opts :: keyword()) ::
          :ready | {:error, :timeout}
  def await_notification(%Octocon.RPC.Postgres.LSN{source: :insert} = lsn, opts \\ []) do
  ...
  @spec request_and_await_notification(
          :wal_lookup_failure | Octocon.RPC.Postgres.LSN.t(),
          opts :: keyword()
        ) ::
          :ready | {:error, :timeout}
  ...
  def request_and_await_notification(%Octocon.RPC.Postgres.LSN{source: :insert} = lsn, opts) do
    ...
    case fetch_request_entries(req_table) do
      ...
      requests ->
        ...
        Enum.each(requests, fn {pid, lsn_insert} = entry ->
          ...
          # entry
          if Octocon.RPC.Postgres.LSN.replicated?(last_replay, lsn_insert) do
