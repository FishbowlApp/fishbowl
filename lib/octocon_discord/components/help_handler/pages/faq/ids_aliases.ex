  ...
  def embeds do
    [
      %Embed{
        ...
        ### Aliases
        Aliases are a way to give your alters a more human-readable ID. You can set an alias for an alter using the `/alter edit` command with the `alias` parameter. For example, `/alter edit id:1 alias:Atlas` would set the alias of the alter with ID `1` to `Atlas`.

        Once an alter has an alias, you can use it in place of their numerical ID in all commands. In the above example, `/alter view id:Atlas` would view the alter with ID `1`.
